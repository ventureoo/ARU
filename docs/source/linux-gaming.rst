...
префикса (вместо ``~/.wine`` указываете желаемый префикс)::

   cp /usr/share/dxvk/x32/*.dll ~/.wine/drive_c/windows/syswow64
   cp /usr/share/dxvk/x64/*.dll ~/.wine/drive_c/windows/system32

Затем необходимо специально указать Wine, чтобы вместо встроенной реализации
...
На этом ручная установка DXVK завершена. Учтите, что графические менеджеры как
Lutris или Proton в Steam уже выполняют установку DXVK за вас, поэтому в этом
случае никаких дополнительных действий выполнять не нужно.

.. warning:: DXVK осуществляет ретрансляцию вызовов только для игр
   использующих версии DirectX 8, 9, 10 и 11. Для DirectX 12 для понадобиться
   использовать vkd3d. Подробнее о нем вы можете прочитать ниже.
...
предварительно скопировать в нужный Wine префикс (в примере это ``~/.wine``,
но может быть любой другой)::

   cp /usr/share/vkd3d-proton/x32/*.dll ~/.wine/drive_c/windows/syswow64
   cp /usr/share/vkd3d-proton/x64/*.dll ~/.wine/drive_c/windows/system32

А также указать форсированное использование vkd3d-proton вместо встроенной в
Wine реализации vkd3d::
