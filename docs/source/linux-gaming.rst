...
- Содержит все патчи, применяемые в wine-staging.
...
- Исправлена работа анти-чита Genshin Impact.

- Исправлена работоспособность некоторых игр и установщиков.
...
proton-cachyos -  это форк проекта Proton от разработчиков дистрибутива
CachyOS для запуска Windows-игр с дополнительными патчами и оптимизациями не
вошедшими в основную ветку Proton, а также улучшение совместимости с
некоторыми играми. Позволяет играть во многие проекты которые не заводятся с
обычным Wine или Proton.

Установить последнюю версию proton-cachyos можно при помощи следующей
команды::

  curl -sSL https://api.github.com/repos/CachyOS/proton-cachyos/releases/latest  |
    grep -oP '"browser_download_url": "\K.*-x86_64.tar.xz(?=")' |
    curl -s -o /dev/stdout -L @- |
    tar --xz -x -v -C ~/.steam/root/compatibilitytools.d/ -f -
...
Дабы использовать proton-cachyos в качестве альтернативы обычному Proton, после
установки proton-cachyos вам нужно перезапустить Steam и зайти в *Свойства*
нужной вам игры, прожать в: *Совместность -> Принудительно использовать
определенный инструмент совместности Steam Play -> proton-cachyos-slr*. Готово,
...
.. warning:: Демоны ananicy и ananicy-cpp вместе с использованием gamemode,
  настроенным с указанием ``renice``,  могут конфликтовать, поэтому не
