...
.. index:: seutp, wine, winewayland
.. _wine_wayland:

""""""""""""""""""""""""""""""""""
Нативная поддержка Wayland в Wine
""""""""""""""""""""""""""""""""""

Начиная с версии 9.0 в Wine появилась поддержка нативной работы запускаемых
Windows приложений в окружениях на базе Wayland. Она представлена в виде
отдельного графического бэкенда, являющегося частью Wine и называемого
winewayland. На текущий момент его функциональность реализована не полностью и
все ещё находится в процессе доработки, однако на текущем этапе его развития
уже возможно запускать игры и получать хороший уровень производительности. Для
того чтобы задействовать работу winewayland в последних версиях Wine нужно
просто сбросить значение переменной ``DISPLAY``. Например, следующая команда
запустит winecfg без использования лишнего слоя совместимости в виде
XWayland::

  DISPLAY= winecfg

В Lutris и иных графических помощниках включить работу winewayland можно так
же просто используя пустое значение для переменной ``DISPLAY``.

Поддержка Wayland также доступна в сторонних сборках Proton при использовании
переменной окружения ``PROTON_ENABLE_WAYLAND=1``.
...
.. index:: lutris, manual, installation
.. _lutris_manual_game_installation:

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Запуск игр устанавливаемых вручную
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

С играми, которые есть в библиотеке того или иного цифрового сервиса, всё
довольно просто - для них Lutris автоматически загружает так называемый
"рецепт" запуска, который по существу представляет собой YAML файл, содержащий
необходимые шаги для загрузки игры, а также инструкции по настройке префикса,
используемого при запуска игры, так что как правило никаких дополнительных
"телодвижений" предпринимать не требуется.

Но встает вопрос что делать с играми, которые нет в цифровой библиотеке? Это
могут быть и физически купленные копии, коллекционные издания или скачанные из
сервисов, с которыми у Lutris нет удобной интеграции. Ну и конечно, пиратские
копии, куда же без них (что мы безусловно осуждаем!).

Для начала следует понять, возможен ли запуска игры через Wine в принципе, то
есть провести предварительный анализ. В этом нам помогут несколько весьма
полезных источников:

- ProtonDB (https://www.protondb.com/) - наиболее популярный ресурс среди
  игроков под Linux. Здесь простые пользователи оставляют свои отчёты о
  запуске тех или иных игр с помощью Proton. Однако как правило если что-то
  запускается через Proton, то это запускается и через обычный Wine, хотя
  иногда требуется выполнить дополнительные шаги по его настройке.
  Воспользуйтесь поиском на сайте, чтобы понять найти нужную игру и выяснить
  какой у игры статус. Статус помогает определить насколько беспроблемно
  работает игра под Linux. Если игра не помечена как "Borked", то как минимум
  вы сможете запустить игру. Статус "Bronze" (бронза) означает, что игра
  запускается, но имеет определенные проблемы (низкая производительность,
  частые аварийные завершения, и т. п.). Часто при этом пользователи сами
  указывают, что помогает игре работать лучше, поэтому важно читать различные
  отчёты. "Gold" (или золото) означает что игра запускается с незначительными
  проблемами, "Platinum" - работает без проблем для подавляющего большинства.

- База рецептов от Lutris (https://lutris.net/games/) - то самое место, откуда
  Lutris подгружает нужные инструкции запуска игры. При запуске локально
  установленных игр можно подсмотреть оттуда нужные команды, названия DLL
  библиотек, требуемую игрой версию Windows и т. п. Чтобы это сделать нужно
  при помощи поиска найти желаемую игру, далее выбрать из списка возможных
  установок *"WINE"* (если есть), затем рядом с кнопкой *"Install"* нажать на
  выпадающую стрелочку, которая откроет контекстное меню. В нём нас интересует
  пункт *"View install script"*, который и покажет все используемые Lutris
  инструкции по установке игры и настройке префикса.

- AppDB WineHQ (https://appdb.winehq.org/) - официальный ресурс от проекта
  Wine, который позволяет проверить статус работы тех или иных игр при помощи,
  собственно, простых, не модифицированных версий Wine. Имеет схожую с
  ProtonDB систему статусов, но в отличии от него содержит информацию о
  запуске не только игр, но и простых приложений. Для каждой игры или
  приложения может быть указан список известных ошибок. Тем не менее,
  информация часто может быть устаревшей, так как статус многих приложений и
  игр редко пересматривается со временем, то есть с использованием последних
  версий Wine.

- PCGamingWiki (https://www.pcgamingwiki.com/) - ресурс, не относящийся
  напрямую к процессу запуска игр под Linux, но содержащий много полезной
  справочной и технической информации об играх, что может служить хорошим
  ориентиром при отладке различного рода проблем.

Как показывает опыт, чтобы понять как успешно (или не очень) запустить
локальную копию игры достаточно информации из первых двух источников. 

Перейдем от слов к делу и рассмотрим классическую схему ручного запуска игры.
Открываем Lutris и нажимаем на значок *"+"* в левом верхнем углу.

.. image:: images/lutris-add-game.png

Далее Lutris спросит в каком виде представлена добавляемая игра.

.. image:: images/lutris-add-game-choice.png

Здесь есть 5 вариантов:

1) Игра устанавливаемая через общедоступный онлайн-установщик. Этот вариант
часто используется для установки и запуска различных онлайн-игр, например
таких как Genshin Impact, World Of Tanks и т. д.

2) Установить игру из EXE установщика на диске. Такой вариант сейчас
чаще всего встречается, когда речь идёт о копиях игр из магазина GOG, которые
распространяются именно в виде классической программы для установки игры, но
это также могут любые другие установщики.

3) Установить игру при помощи готового "рецепта". По умолчанию все YAML
конфигурации для запуска игр загружаются из общей базы данных Lutris, но вы
также в праве написать свой для нужной вам игры, однако данный вариант не
будет рассматриваться в рамках раздела, так как имеет весьма узкую специфику.

4) Импортировать ROM файл. Используется в основном для очень старых игр или
сдампленных физических копиях. В данном разделе рассматриваться не будет.

5) Добавляет локально установленную игру. Тот самый вариант, который мы и
будем использовать чаще всего в случае, если игра находится уже в
распакованном виде где-то на диске, но вне нашей цифровой библиотеки.

В зависимости от того какой из вариантов вы выберите в диалоговом окне будет
зависеть дальнейший набор шагов:

.. tab-set::

   .. tab-item:: Общедоступный онлайн-установщик

      Один из самых простых вариантов при выборе которого Lutris спросит
      название игры, которую вы хотите установить. Рассмотрим весь процесс на
      примере бесплатной онлайн-игры World Of Tanks. Вводим название и
      выбираем нужную из списка:

      .. image:: images/lutris-install-online-game-1.png

      Далее для некоторых игр требуется указать тип установки, в нашем случае
      World Of Tanks предоставляет несколько вариантов в зависимости от
      региона. Выберем универсальный тип для всех регионов:

      .. image:: images/lutris-install-online-game-2.png

      В следующем окне Lutris спросит директорию для установки игры. По
      существу это будет расположение префикса с игрой:

      .. image:: images/lutris-install-online-game-3.png

      Затем в зависимости от конкретной игры могут появится дополнительные
      диалоги выбора. Так как изначально в качестве типа используемой
      программы установщика мы выбрали универсальный вариант, теперь Lutris
      спросит нас о том, для какого именно региона мы хотим скачать установщик:

      .. image:: images/lutris-install-online-game-4.png

      После всех дополнительных шагов начинается процесс загрузки установщика,
      а также настройки префикса под игру, так как установка игр через данный
      способ добавления внутрь библиотеки, производится с использованием
      готового рецепта из базы данных Lutris, о чем нам указывает окно вывода
      сообщений из журнала.

      .. image:: images/lutris-install-online-game-5.png

      Наконец должно появится окно с установщиком прямо как в Windows.
      Дальнейшие действия целиком зависят от конкретной игры, в нашем случае
      появившаяся программа, запущенная через версию Wine, выбранную в
      глобальных настройках Lutris, просто установит внутрь префикса с игрой
      лаунчер для её запуска, поэтому никаких других диалогов с вопросами не
      будет.

      .. image:: images/lutris-install-online-game-7.png

      В редких случаях Lutris может спросить пользователя о каких-то
      настройках применяемых для самой игры, которые будут применены перед
      окончанием процесса её добавления в библиотеку. В нашем случае - это
      выбор используемой локализации.

      .. image:: images/lutris-install-online-game-8.png

      Lutris сразу же предложит запустить игру после окончания процесса.

      .. image:: images/lutris-install-online-game-9.png

      Как можно заметить, процесс установки оказался довольно простым и
      сводиться к привычному прокликиванию кнопки "Далее". Никакой головной
      боли с настройкой префикса или переменных окружения под игру нет, так
      как всё удобным образом автоматизировано.

      .. image:: images/lutris-install-online-game-10.png

   .. tab-item:: Установка при помощи EXE файла

      ::

   .. tab-item:: Локально установленная копия
