...
``_staging_version="v9.20"`` - Устанавливает желаемую для сборки версию Wine.
...
Чтобы избежать использования Xwayland по умолчанию нужно будет сбросить
значение переменной ``DISPLAY``, то есть оставить её без значения перед
запуском Wine::
...
.. warning:: Данная переменная окружения не работает при использовании WoW64:
   https://github.com/Frogging-Family/wine-tkg-git/pull/1282
