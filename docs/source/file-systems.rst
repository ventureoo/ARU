...
.. note:: На накопителях, объемом более терабайта, *space_cache* можно заменить
   на *space_cache=v2*, что тоже даст определенную прибавку производительности.
...
.. attention:: Параметр ``commit=600`` может вызывать повышенное потребление
   оперативной памяти и портить данные на диске.
