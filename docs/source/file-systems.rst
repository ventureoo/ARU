...
Предложим перечень наиболее безопасных и оптимальных флагов для SSD
носителей::

 rw,relatime,ssd_spread,commit=300

Для HDD достаточно убрать флаг ``ssd_spread`` и добавить вместо него
флаг для автоматической дефрагментации ``autodefrag``, так чтобы не
возникало необходимости выполнять её вручную::

 rw,relatime,commit=300,autodefrag

Прежде всего, отметим, что вы можете изменить ``relatime`` на
``noatime`` или ``lazytime`` - все три параметра отвечают за
запоминание времени обращения к файлам.

Параметр ``noatime`` полностью выключает данную функцию, что может
привести к ошибкам в приложениях зависимых от времени изменения файлов
(например git), однако это встречается довольно редко. Параметр
``lazytime`` успешно будет выполнять все функции времени, но выполнять
их промежуточную запись в оперативной памяти, что позволит избежать
замедления без потери функционала, однако ``lazytime`` может иметь
свои проблемы в связи с реализацией, так как при данные могут быть
утеряны при аварийных отключениях питания.
...
Согласно `документации Btrfs
<https://btrfs.wiki.kernel.org/index.php/Compression>`_, официально
поддерживается 3 алгоритма сжатия без потерь:
...
отредактировать файл ``/etc/fstab``, добавив для необходимого раздела
с файловой системой Btrfs следующий флаг монтирования::
...
выше флаги монтирования::
  ...
  rw,relatime,compress=zstd:3,ssd_spread,commit=600
...
* С учетом разницы в экономии места порядка **3 GB** между максимальной доступной
  ...
  Согласно последним `тестам <https://gist.github.com/braindevices/fde49c6a8f6b9aaf563fb977562aafec>`__, чтобы использовать NVME SSD так, 
  чтобы не сильно сокращать скорости чтения/записи - рекомендуется
  использовать алгоритм сжатия ``zstd`` с уровнем 1, т.е. ``zstd:1`` или ``lzo``. Более низкий уровень сжатия создает меньшую нагрузку на 
  CPU, тем самым возникает меньше упора в процессор при работе с быстрыми
  NVME SSD.
  Это также можно использовать для SATA SSD, однако из-за более
  низкой скорости работы интерфейса SATA можно увеличивать уровень
  сжатия ``zstd``, например до **3**. Такое увеличение позволяет увеличить
  производительность из-за передачи меньшего количества данных.
