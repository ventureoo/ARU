...
.. index:: installation, x86_64_v3, cachyos, repository, packages
.. _cachyos_repository:
...
К счастью, дистрибутивом CachyOS развивается `собственный репозиторий
<https://wiki.cachyos.org/features/optimized_repos/#adding-our-repositories-to-an-existing-arch-linux-install>`_,
который содержит **все пакеты** из официальных репозиториев, но
собранных для процессоров x86_64_v3 или x86_64_v4. То есть это те же
самые, уже готовые пакеты из официальных репозиториев, но собранные с
различными оптимизациями для определенной группы процессоров
(поколений x86_64).
...
.. danger:: Оптимизированные пакеты для процессоров Intel поддерживают
   только "полноценные" процессоры серий Core 2 и i3/i5/i7. Многие
   процессоры Pentium/Celeron не имеют полного набора инструкций
   (например могут быть порезаны AVX инструкции), необходимого для
   использования оптимизированных пакетов. Пользователям этих
   процессоров не следует подключать репозитории с данными пакетами,
   даже если GCC возвращает значение, соответствующее полному набору
   флагов процессоров Intel Core i3/i5/i7, например, Haswell.
...
Если у вас поддерживается хотя бы ``x86_64_v3``, то вы так же сможете
использовать данные репозитории, ибо он предоставляет пакеты как для
``x86_64_v3``, так и для ``x86_64_v4``. **Главное не перепутаете,
какое именно у вас поколение**.

Чтобы подключить репозиторий установим ключи для проверки подписей
пакетов и список зеркал::
...
После этого нужно отредактировать конфиг pacman (``/etc/pacman.conf``)
добавив репозитории под максимально поддерживаемую архитектуру вашим
процессором выше обычных репозиториев Arch Linux:
