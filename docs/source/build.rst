...
<https://somegit.dev/ALHP/ALHP.GO>`__, который содержит **все пакеты**
из официальных репозиториев, но собранных для процессоров x86_64_v2
или x86_64_v3. То есть это те же самые, уже готовые пакеты из
официальных репозиториев, но собранные с различными оптимизациями для
...
для нужной архитектуры (``/etc/pacman.conf``).
...
.. note:: Будьте бдительны при использовании ключей ``-uu`` вместе с
   AUR помощниками, это может вызвать проблемы с откатом ``-git``
   пакетов до тех версией, которые указаны на сайте AUR, а не
   настоящих, которые получаются после их сборки.
