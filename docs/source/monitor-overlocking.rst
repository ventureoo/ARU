...
.. index:: monitor, settings, kde, plasma
.. _kscreen_overclocking:

=======================================
Способ для окружения KDE Plasma (6.6+)
=======================================

В окружении KDE Plasma начиная с версии 6.6 появился более простой способ
"разгона" [#]_, а точнее возможность добавления собственного режима отображения при
помощи утилиты ``kscreen-doctor``, например::

  kscreen-doctor output.1.addCustomMode.1920.1080.145000.full

Это добавит в настройки окружения возможность выбора режима развёртки в 145 Гц
(здесь они указаны в милигерцах) вместо стандартных 144 для экрана под номером
1 и при разрешении 1920 на 1080. Узнать номер нужного экрана если их несколько
можно при помощи команды ``kscreen-doctor -o``.

Удалить добавленный таким образом режим развёртки можно через следующую
команду::

  kscreen-doctor output.1.removeCustomMode.0

Данный способ применим вне зависимости от текущего драйвера GPU и позволяет
применять изменения на лету, поэтому он предпочтителен к использованию вместо
"классического" способа с редактированием EDID, конечно если вы используете
окружение от проекта KDE.
