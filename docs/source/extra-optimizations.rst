...
:text:`sudo nano /etc/default/cpupower` # Редактируем строчку ниже

*governor=’performance’* # Высокая производительность

:text:`sudo systemctl enable cpupower` # Включить как постоянную службу, которая установит вечный perfomance mode.
...
:text:`sudo pacman -S polkit`  # Установить для управления системными привилегиями.

:text:`sudo nano /etc/polkit-1/rules.d/10-disable-suspend.rules`  # Убираем спящий режим и гибернацию (из меню и вообще). Если этого файла нет, то создать. Файл должен выглядеть вот так:
