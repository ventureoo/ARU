...
В Linux можно уменьшить количество обращений к хранилищу данных (HDD или SSD) если перенести директорию ``.cache`` в ОЗУ. Для этого у вас директория ``/tmp`` должна использовать *tmpfs* (Arch использует по умолчанию), для проверки::
...
1) Нужно создать скрипт (например ``~/.local/bin/cache_move.sh``) со следующим содержимым:: 
...
3) Настроить выполнение скрипта:
  - через systemd 
