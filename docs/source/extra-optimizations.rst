...
В Linux можно уменьшить количество обращений к хранилищу данных (HDD или SSD) если перенести директорию ``.cache`` в ОЗУ. Для этого у вас директория ``/tmp`` должна использовать *tmpfs* (Arch использует по умолчанию).
