...
Данный раздел предназначен для разных скриптов, которые могут быть
полезными или просто познавательными. Например, если вы хотите
отказаться от использования панелей / баров, то часть информации вам
может пригодиться.
Для запуска скриптов в WM вам нужно назначить горячие клавиши,
указать команду выполнения и прописать путь до скрипта. Например в *river*::

  riverctl map -layout 0 normal Alt T spawn '$HOME/.local/bin/time.sh' 

.. index:: scripts, time_to_notify
.. _script_time:

===========================================
Вывод текущей даты и времени в уведомление
===========================================

Дату и время можно вывести в качестве увидомления на экран, если в WM
обозначить комбинацию клавиш и создать скрипт, например:
...
.. code-block:: shell 
  ...
  notify-send -a "" "Текущее время" "$(date '+ \n Дата: %d.%m.%Y \n\n Время: %H:%M:%S')"

При вызове данного скрипта будет направлена информация о текущей дате
и времени.

.. warning:: Не забудьте, что для работы уведомлений в WM вам
   потребуется демон уведомлений, например dunst или fnott. 

.. index:: volume, volume level
.. _script_volume:
...
Для отображения уровня громкости при изменении громкости можно
использовать подобный скрипт.
...
.. code-block:: shell  
  ...
  case "$1" in
    -1)  $(wpctl set-volume @DEFAULT_SINK@ 2%-)  ;;
    0)   $(wpctl set-mute @DEFAULT_SINK@ toggle)  ;;
    1)   $(wpctl set-volume @DEFAULT_SINK@ 2%+)  ;; 
  esac

  volume=$(wpctl get-volume @DEFAULT_SINK@ | sed 's/Volume://g')

  if [ !$(wpctl get-volume @DEFAULT_SINK@ | grep "MUTED") ]
  then
    notify-send -t 2000 -h string:x-dunst-stack-tag:volume-notif -h int:value:$(bc<<<$volume*100) "Уровень громкости  " "\n$(bc<<<$volume*100) %"
  else
    notify-send -t 2000 -h string:x-dunst-stack-tag:volume-notif "Уровень громкости" "\n  Звук выключен"
  fi

Для использования данного скрипта необходимо назначить горячие клавиши
и передавать параметр изменения -1, 0, 1. Запуск из терминала выглядит
так::

  ./vol.sh 1    # увеличить уровень громкости 
  ./vol.sh 0    # включить или выключить звук
  ./vol.sh -1   # уменьшить уровень громкости

По умолчанию шаг изменения громкости выставлен на 2%, если вы хотите
это изменить - исправьте **2** в **2%-** и **2%+** на другое значение. 

.. warning:: В данном скрипте используются иконки из **Nerd-Fonts**,
   если они не отображаются правильно, то установите шрифт!  

.. warning:: Данный скрипт использует некоторые особенности dunst,
   поэтому он необходим! Возможно подобные функции можно реализовать и
   в других демонах уведомления.

.. index:: sound, input-sound, output-sound
.. _script_sound:

======================================
Выбор устройства ввода и вывода звука
======================================

Есть такой лаунчер - *dmenu*. Отличительной его особенностью была в
том, что в него можно было выводить разную информацию. Благодаря этому
можно было реализовывать разные подобия меню. Многие лаунчеры
унаследовали такую возможность. На примере *fuzzel* будет
продемонстрировано как реализовать выбор звукового устройства с
помощью **wpctl**, в качестве вывода информации будет использоваться
**notify-send**.
...
.. code-block:: shell  
  ...
  lines=$(wpctl status | grep -oP '\K([0-9]+.*)(?=[vol*)')

  select=$(printf '%s\n' "${lines[@]}" | fuzzel -p "Звук > " -w 70 -d "$@")

  [[ -n $select ]] || exit
  
  wpctl set-default ${select%.*}
  notify-send "Устройство звука:" "\n   ${select//[0-9]*. }"
...
.. warning:: На данный момент в скрипте нет разделения на устройства
   ввода и вывода звука, поэтому они находятся вместе! Вначале как
   правило идут устройства вывода, а потом устройства ввода, также как
   и при использовании **wpctl status**. 
