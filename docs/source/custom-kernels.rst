...
**Установка I.**
.. warning:: Данный блок писал основной мейнтейнер ядра, поэтому небольшое предупреждение
   В данном ядре нет поддержки нативной компиляции как таковой, ядро расчитано под Generic и его поколения. Автором поддерживается лишь **бинарные** ядра
   В данном ядре **никогда** не будет поддержки modprobed-db из-за проблем которые могут выйти после сборки, поэтому данное ядро собирается во всеми модулями

Теперь приступим к сборке из PKGBUILD, для этого вам для начала нужно
проверить микроархитектуру вашего процессора::

  /lib/ld-linux-x86-64.so.2 --help | grep "x86_64* (supported, searched)"

Данная команда выводит все доступные архитектуры вашего процессора
...
Сборка ядра производиться данными командами::

  git clone https://gitlab.com/hybrid-project-developers/pkgbuilds/hybrid-core-pkgbuilds/kernel/linux-hybrid.git  # Скачиваем исходники
  cd linux-hybrid
  export _build_cpu_opt="" # Здесь смотрим какую микроархитектуру поддерживает ваш процессор - если лишь x86_64, указыаваем generic, если x86_64v2 - generic_v2, а если x86_64v3 - generic_v3
  makepkg -sric
.. warning:: В PKGBUILD имеется баг из-за которого вызывается базовая конфигурация модулей
   Это связано с установкой 1000Hz на динамические тики
   Просто прожимайте клавишу Enter когда она появиться, также игнорируйте выбор архитектуры процессора

Вам остается откинуться на спинку кресла и насладиться процессом
