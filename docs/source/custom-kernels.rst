...
:bash:`_futex2="true"` - Осуществляет использование нового, экспериментального futex2 вызова что может дать лучшую производительность для игрушек запускаемых через Wine/Proton. Для обычных ядер поддержка Futex2 включена начиная с версии 5.16+.

:bash:`_winesync="false"` - Ещё одна замена esync, но уже от разработчиков Wine.
...
:bash:`_complierlevel="1"` - Задает степень оптимизации ядра во время сборки.
...
:bash:`_ftracedisable="true"` - Отключает лишние трекеры для отладки ядра.
