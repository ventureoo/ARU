...
.. warning:: Данный способ является временным решением, т.к. ещё не
   ...
   и/или зарегистрироваться на Github что бы скачать пакеты
...
перед этим нужно проверить микроархитектуру, как мы делали это при
обычной сборке ::
...
- Если команда ничего не выдала - процессор поддерживает лишь базовую
  x86_64.
...
**2.** Откройте последний успешний Workflow (помечен зеленой
галочкой).

.. image:: images/linux-hybrid-action-page.png

**3.** Откройте его и прокрутите вниз, там 3 артефакта, выше мы уже
выполнили команду для проверки микроархитектуры, отталкиваясь от её
выхлопа выбирайте необходимый вам артефакт.
