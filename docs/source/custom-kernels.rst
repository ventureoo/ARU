...
``_version="6.0"`` - Здесь выбираем версию ядра которую мы хотим
...
"cfs" (дефолтный для ванильного ядра), "bore". По некоторым данным,
PDS дает больше FPS, а CacULE дает лучшие задержки по времени кадра
...
``_tickless="1"`` - Рекомендуется выбирать Полные Динамические тики
таймера ядра. Подробнее об данной настройке вы можете прочитать в
разделе "Настройка ядра".
