...
[#]_. Работа по поиску "дубликатов" реализована при помощи фонового
...
.. code-block:: shell
  :caption: ``/etc/tmpfiles.d/ksm.conf``
...
выбранного региона, но и память всего процесса целиком [#]_.
...
используя опцию ``MemoryKSM=yes`` [#]_, но так как системный менеджер
...
.. code-block:: shell
  :caption: ``/etc/systemd/system/user@.service.d/10-ksm.conf``

  [Service]
  MemoryKSM=yes

После сохранения изменений и перезапуска системы проверить
корректность применения атрибута можно при помощи специальной
программы, исходный код которой приложен ниже:

.. code-block:: c
  :caption: ``/tmp/ksmtest.c``
...
Скомпилировав и запустив данную программу можно понять
распространяется ли работа KSM для пользовательских процессов::
...
То всё работает и можно получить статистику о текущем общем объеме
сэкономленной памяти в мегабайтах при помощи следующей команды::
...
Вывод должен остаться таким же, как и в случае с глобальным
применением KSM. В целом же использование данного механизма
рекомендуется в программах связанных с обработкой большого количества
произвольных пользовательских данных, которые часто содержат чрезмерно
много повторяющихся элементов. Примерами можно назвать: браузеры (и
основанные на их движках приложения), виртуальные машины и даже
видеоигры.
