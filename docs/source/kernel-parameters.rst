.. ARU (c) 2023 - 2025, Vasiliy Stelmachenok and contributors
...
-----------------------------
Настройка планирования CPU
-----------------------------

Прежде чем мы начнем говорить непосредственно о настройке, как и
всегда необходимо ввести некоторую теоретическую базу, на которую мы
будем опираться в дальнейшем. Итак, для начала что же такое
"планирование CPU"? Ведь в понимании большинства людей CPU это нечто
неделимое, атомарное, в отличии от той же памяти, где мы можем
выделить определенный кусок ОЗУ и работать с ней так же, как и со всей
памятью вообще. В случае с CPU под планированием в многозадачных ОС, к
которым относится и Linux, понимается распределение *процессорного
времени*, которое получают процессы во время работы системы. То есть
это отношение N готовых к выполнению процессов к 1 одному CPU, и нужно
выбрать, какое из N готовых следует выполнять процессору прямо сейчас.
Здесь стоит сразу оговориться, что в современных системах данное
отношение применяется не к CPU в целом, а к каждому из его ядер,
поэтому каждое ядро имеет собственную выборку процессов, готовых к
выполнению.

Если вдруг раньше по какой-то причине вы думали, что все ваши процессы
в системе выполняются полностью параллельно и независимо друг от
друга, то как мы уже выяснили это не совсем так. Единовременно каждое
ядро CPU может выполнять только какую-то одну задачу (процесс или
нить/поток), поэтому здесь имеет место скорее конкуренция за
"внимание" со стороны CPU. 

Стоит отметить, что в Linux планирование осуществляется не просто над
процессами, как это принято в классических системах Unix, но вместо
этого над их потоками (thread). То есть по сути ядро занимается
планированием именно потоков среди разных процессов, так как даже если
приложение не использует многопоточность, то его процесс всё равно
включает в себя хотя бы один поток (главный). Но вообще говоря в
документации Linux принято обозначать за единицу планирования
некоторую *задачу*, под которой может пониматься как процесс, так и
поток, поэтому в дальнейшем мы будем использовать именно этот термин.

При этом каждая задача в общем случае может находится в одном из трёх
состояний: быть выполняемой в данный момент, готовой к выполнению и
находится в состоянии сна. О первом состоянии мы уже говорили,
состояние сна обычно же наступает тогда, когда задача начинает ожидать
завершения какого-то события, к примеру окончания записи или чтения
информации с диска, в этом случае CPU такой задаче становится не
нужен, поэтому до тех пора пока она остается в этом состоянии никакого
процессорного времени ей выделено не будет. По завершении этого
события задача становится готовой к выполнению.
...
Изменение состояния каждой задачи тесно связано с понятием *таймера
прерываний* (timer interrupt). Его работа заключается в том, чтобы всё
время с некоторой постоянной частотой генерировать прерывания. После
осуществления прерывания ядро приостанавливает текущую выполняемую
задачу и может начать операцию перепланирования, то есть выбора
следующей задачи для выполнения. Это может быть та же самая задача,
которая выполнялась до прерывания, а может быть какой-то другой из N
готовых к выполнению в данный момент. Какая именно задача будет
выполняться дальше зависит от используемого алгоритма (политики)
планирования, о которых пойдет речь далее. Стоит отметить, что при
использовании некоторых политик планирования ядро может выполнить
вытеснение выполняющейся задачи гораздо раньше, чем будет выполнено
прерывание таймером, однако другие задачи, которые находятся под
управлением так называемых политик реального времени по прежнему будут
строго привязаны к частоте таймера прерываний.

Частота таймера прерываний задается на этапе сборки ядра через
значение параметра ``CONFIG_HZ`` и её невозможно изменить ни на этапе
загрузки ядра, ни тем более во время его работы. В большинстве
дистрибутивов Linux это значение равно ``1000``, что является хорошим
и рекомендуемым значением для настольных систем. Проверить какая
частота таймера прерываний используется в вашей системе можно через
следующую команду::

  zgrep "CONFIG_HZ=" /proc/config.gz

Официальные сборки ядра на момент написания данного раздела,
поддерживают всего 4 возможных значения: ``100``, ``250``, ``300`` и
``1000``. Чем меньше указанное значение, тем реже таймер будет
генерировать прерывания, и следовательно тем больше процессорного
времени будут получать задачи прежде, чем они будут прерваны, что
позволяет получить большую пропускную способность. И наоборот, чем
больше частота таймера, тем чаще будут происходить прерывания и меньше
процессорного времени будут получать задачи в самом общем случае, но в
то же время это позволяет за тот же самый промежуток времени большему
количеству задач получить это процессорное время, то есть понижается
задержка, через которую задача приступают к работе на CPU.

Собственно именно поэтому высокая частота таймера равная ``1000`` и
является более предпочтительной для домашних настольных систем, так
как интерактивность и отзывчивость на них являются более важным
апспектом, чем пропускная способность.

Кроме частоты таймера во время сборки ядра также настраивается его
поведение. Хоть мы и сказали о том, что таймер прерываний работает с
постоянной частотой, и это правда так, но при некоторых условиях
работа таймера прерываний может просто не происходить. Это зависит от
используемых параметров ``CONFIG_NO_HZ``. Посмотреть какие из них
включены для вашего ядра можно через следующую команду::

  zgrep "CONFIG_NO_HZ*" /proc/config.gz

Набор данных параметров стоит рассматривать хронологически или же
"эволюционно". Изначально в ранних версиях ядра таймер генерировал
прерывания абсолютно всегда при любых условиях. Сейчас он тоже может
это делать при указании в конфиге ядра ``CONFIG_NO_HZ=n`` или
``CONFIG_HZ_PERIODIC=y``. Это хорошо работает для тех систем, где всё
время есть большое количество каких-то задач, то есть процессов и
потоков, между которыми нужно переключаться процессору. Но такие
ситуации достаточно редкие, к примеру на большинстве домашних систем
ваш процессор не имеет какую-то постоянную нагрузку, рано или поздно
наступает период "покоя", когда просто большая часть ядер CPU
перестает быть нагруженной, так как все процессы спят или выполняют
кратковременные операции. В таком случае таймер прерываний выполняет
по сути холостую работу, которая может сильно повысить
энергопотребление простаивающих без дела систем.

Чтобы решить указанные проблемы, разработчики ядра сделали новый режим
работы таймера ``CONFIG_NO_HZ_IDLE``. С ним таймер делает всё то же
самое, что и раньше, но просто не генерирует прерывания в тех случаях,
когда ядро CPU простаивает, то есть нет задач готовых к выполнению.
Идея не делать лишних прерываний оказалась довольно удачной, так что
позже появился ещё один режим ``CONFIG_NO_HZ_FULL``. При его
использовании таймер прерываний перестает работать не только когда
ядро CPU простаивает, но и тогда, когда ядро CPU имеет в данный момент
всего одну задачу готовую для выполнения, и которая собственно и
выполняется в данный момент. И действительно, если задача только одна,
то и прерывать его нет никакого смысла.

Стоит отметить, что для использования данного поведения недостаточно
просто указать в конфиге ядра ``CONFIG_NO_HZ_FULL=y``, нужно также
передать при запуске параметр ``nohz_full``, с перечислением номеров
тех ядер процессора, на которые оно будет распространяться. Например,
``nohz_full=2,5``, включит пропуск прерываний на 3 и 6 ядре
(естественная нумерация ядер CPU отличается на единицу, так как в
параметре ядра принимаются начиная с 0, то есть CPU0 - первое ядро,
CPU1 - второе ядро и т.д.). Без указания ``nohz_full`` ядро с
включенным ``CONFIG_NO_HZ_FULL`` работает так же, как и ядро с
``CONFIG_NO_HZ_IDLE``. В большинстве дистрибутивов Linux используют по
умолчанию либо ``CONFIG_NO_HZ_IDLE``, либо ``CONFIG_NO_HZ_FULL``. Как
правило вам не нужен параметр ``nohz_full``, только если вы не
используете его вместе с изоляцией ядер CPU для запуска на них
чувствительных к задержкам задач.

.. index:: sched, cpu, policies
.. _index: sched_policies:

~~~~~~~~~~~~~~~~~~~~~~
Политики планирования
~~~~~~~~~~~~~~~~~~~~~~

После получения прерывания (преждевременно или через таймер
прерываний) перед планировщиком ядра стоит задача выбора следующей
задачи на выполнение. То какая именно задача получит доступ к CPU
следующей зависит от используемой политики планирования. Каждая задача
планируется в соответствии со своей политикой, которая подразумевает
под собой определенный алгоритм. Всего их в ядре насчитывается на
данный момент 6: ``SCHED_FIFO``, ``SCHED_RR``, ``SCHED_DEADLINE``,
``SCHED_IDLE``, ``SCHED_BATCH``, ``SCHED_OTHER``. Начиная с версии
ядра 6.12 существует также политика-маркер ``SCHED_EXT``, но она будет
рассмотрена позже в разделе про динамически загружаемые планировщики
задач (``sched_ext``). Первые же 6 политик можно условно разделить на
две большие группы:

1. Политики задач реального времени (``SCHED_DEADLINE``, ``SCHED_RR``,
``SCHED_FIFO``).

2. Политики обыкновенных задач (``SCHED_OTHER``, ``SCHED_IDLE``,
``SCHED_BATCH``)

Политики задач реального времени получают процессорное время в первую очередь,
в то время как политики обыкновенных задач - по остаточному принципу,
если нет задач готовых к выполнению среди политик реального времени.

В целом разделение всех задач на обыкновенные и реального времени -
весьма условно и строго никак не определено. Пользователь сам в праве
решать какие задачи в его системе относить к первой группе, а какие ко
второй, но понятно, что, например, процессы связанные с работой
пользовательского окружения мы скорее будем относить к задачам
реального времени, так как в следствии их интерактивности мы хотим
получить минимальное время отклика. В то же время в системе со
служебным назначением мы так же можем хотеть, чтобы всё процессорное
время было направлено на выполнение какого-то ограниченного набора
задач, но уже не обязательно интерактивных. В этом отношении главное
понимать иерархический принцип, согласно которому все задачи получают
процессорное время в соответствии с их приоритетом.
...
Различия между "обычными" политиками и политиками реального времени
находят себя и в разграничении системы приоритетов. В Linux для задач
реального времени используется некоторый статический приоритет
(rt_prio), представляющий собой число в диапозоне от 0 до 99. Здесь
всё интуитивно понятно: чем выше приоритет, тем скорее задача будет
обслужена и, следовательно, тем больше она получит процессорного
времени.

Для обыкновенных же задач статический приоритет равен нулю, и вместо
этого в рамках политик ``SCHED_OTHER`` и ``SCHED_BATCH`` планирование
осуществляется с использованием относительного приоритета, или так
называемого уровня "любезности" (nice), который находится в диапозоне
от +19 до -20. При этом чем меньше так называемый уровень "любезности"
у задачи, тем более "жадным" он становится по отношению к другим
задачам, потому получает больше процессорного времени. И наоборот, чем
больше числовое значение nice у задачи, тем больше она будет уступать
процессорное время другим и меньше получать его сама.

В отличии от статического приоритета, уровень любезности не
декларирует задачам с более низким значением nice полное доминирование
над задачами с более высоким значением. Даже самое высокое значение
nice гарантирует получение процессорного времени, какими бы
прожорливыми не оставались другие потоки.

На самом деле и статический, и относительный приоритет неявным образом
отображается ядром в некоторое абсолютное значение. Такой приоритет
называют ещё внутренним из-за отсутствия способов прямого изменения.
Его значение заключено в диапозоне от 0 до 139 и чем ближе оно к нулю,
тем больший вес имеет задача. Статический приоритет для задач
реального времени напрямую отображаются на диапозон от ``0`` до ``99``
в *обратном порядке*. Это значит, что статический приоритет 99 будет
равным значению 0 абсолютного приоритета, аналогично ``98`` будет
равен ``1`` и так далее. Значение же nice занимает диапозон от ``100``
до ``139``. При этом его отображение выполняется не к 1:1, поэтому
один и тот же уровень nice может приводить к получению разных значений
абсолютного приоритета в зависимости от решения планировщика.

.. image:: https://blogs.oracle.com/content/published/api/v1.1/assets/CONT7E6E157EFF004C1C9864737324E44A84/Medium?cb=_cache_b8a7&format=jpg&channelToken=3189ef66cf584820b5b19e6b10792d6f
   :align: right

  (Copyright © 1995, 2025, Oracle and/or its affiliates.)

Рассмотрим далее каждую из представленных политик подробнее в порядке
их приоритета.
...
Это политика реального времени, задачи которой получают процессорное
время всегда в первую очередь из-за чего все они имеют статический
приоритет ``-1``. В основе данной политики лежит алгоритм на базе GEDF
(Global Earliest Deadline First) и CBS (Constant Bandwidth Server),
который включает в себя 3 основных понятия: время выполнения задачи
(runtime), крайний срок выполнения задачи (deadline) и период
выполнения задачи (period).

Временем выполнения задачи называют суммарное количество процессорного
времени, которое гарантированно может получить задача в заданный
периода времени, но до истечения времени крайнего срока. Чтобы это
было чуть нагляднее, рассмотрим работу политики на примере.

Допустим, что у вас есть задача, которая успешно обрабатывает кадр из
видео потока в течении 5 ms. Стоит отменить, что за время выполнения
задачи лучше всего принимать самый худший случай, то есть максимальное
время обработки пакета, а не среднее его значение. Так вы сможете
гарантировать, что хотя бы один кадр в течении всего периода будет
обработан. Сам период можно установить на время равное промежутку
времени необходимому для отображения одного кадра на экран, скажем, на
дисплее с частотой обновления 60 Гц это будет равным примерно 16.6 ms.
Крайний срок выполнения задания можно установить в 10 ms.

Здесь возникает закономерный вопрос, зачем делать время крайнего срока
меньше, чем время периода? В общем случае, когда у вас есть всего одна
задача, это и в самом деле имеет мало смысла, но когда у вас есть и
другие задачи с ``SCHED_DEADLINE``, то разница между крайним сроком и
периодом оставляет свободное "окно", во время которого другие задачи
могут начать выполнение, например задача на отрисовку управляющих
элементов плеера.

Таким образом, ``SCHED_DEADLINE`` позволяет явно контролировать график
выполнения задач во времени. При этом вместо использования
классической приоритетной системы, ядро будет всегда выбирать первым
на исполнение задачу с ближайшим крайним сроком. Очевидно, что
использование данной политики весьма ситуативно и покрывает
специфичный набор задач, для которых мы точно можем рассчитать
оптимальное время и период выполнения. Чаще всего это возможно сделать
лишь для встраиваемых систем узкого назначения, поэтому в большинстве
случаев вы не будете использовать данную политику, но полезно знать о
её наличии, так как для заданных целей она позволяет повысить
...
Но вернемся к классическим (согласно POSIX) политикам реального
времени, задачи которых будут выполняться сразу после задач с крайним
сроком в соответствии со своим статическим приоритетом. ``SCHED_FIFO``
работает по достаточно тривиальному алгоритму FIFO (First In-First
Out), представляющему собой обыкновенную очередь, прямо как в магазине
или поликлинике. Это значит, что как только у нас появляется задача со
``SCHED_FIFO``, готовая к выполнению, то следующая появившаяся задача
будет вынуждена ожидать завершения работы первой, либо момента когда
она освободит процессор, например осуществляя операцию ввода/вывода. В
этом случае первая задача перейдёт в конец очереди, и будет ожидать
завершения работы других задач в очереди.

Важно помнить, что алгоритм FIFO работает только в рамках одного
приоритета. В случае если появляется задача с более высоким
приоритетом, то для её выполнения ядро принудительно вытеснит текущую
задачу, и вернётся к ней только после того, как будут приостановлены
все задачи с более высоким приоритетом.
...
``SCHED_RR`` является практически полной копией ``SCHED_FIFO``, за
исключением одной детали - наличие квоты на процессорное время,
превышение которой приводит к преждевременному вытеснению задачи и
помещению её в конец очереди. При этом, если задача была
приостановлена переключением на задачу с более высоким приоритетом, то
при возвращении к прошлой задаче с меньшим приоритетом процесс
дорасходует то процессорное время, которое у неё было до прерывания.

Ограничение по использованию процессорного времени позволяет
убедиться, что выполняемая задача не будет занимать процессор слишком
долго и уступит место другим не менее важным задачам. Надо отметить,
что общее ограничение также присутствует для всех задач реального
времени и представлено значением параметра sysctl
``kernel.sched_rt_runtime_us`` в микросекундах, по умолчанию равное
``0.95`` секунды. Значение данного параметра тесно связано с понятием
периода активации задач реального времени -
``kernel.sched_rt_period_us``, по умолчанию оно равно одной секунде.
Это сильно напоминает ситуацию, которую мы рассматривали говоря о
``SCHED_DEADLINE``: выделяемое процессорное время остаётся меньше
периода активации, чтобы позволить другим задачам начать работу. В
данном случае речь о задачах, не относящихся к политикам реального
времени, что позволяет избежать ситуации "голодания", при которой
задачи реального времени монополизирует работу с процессором, не
оставляя другим потокам возможности приступить к работе. 
...
Если в моменте времени нет задач реального времени готовых к
выполнению, то ядро начинает заниматься планированием обычных задач в
соответствии с политикой ``SCHED_OTHER``. Как правило в неё входит
наибольшее число процессов в системе, так как эта политика
используется по умолчанию. Во многом именно поэтому часто просто
планировщиком CPU в ядре называют именно тот алгоритм, который
контролирует данный набор задач. До версии ядра 6.12 это был CFS
(Completely Fair Scheduler), провозглашавший "справедливое" разделение
процессорного времени между задачами одного приоритета. После версии
6.12 планировщиком по умолчанию стал алгоритм EEVDF (Earliest Eligible
Virtual Deadline First). Несмотря на это полезно рассмотреть сначала
алгоритм работы CFS, так как EEVDF во многом базируется на коде своего
предшественника и является скорее расширением к нему, нежели
полноценной заменой.

В основе идеи CFS по справедливому распределению процессорного времени
лежит понятие *виртуального рабочего времени* (``vruntime``). Оно
учитывает количество времени в наносекундах, которое задача уже должна
была получить и таким образом осуществляется контроль за тем, чтобы
значение ``vruntime`` текущей задачи не стало больше, чем у других в
соответствии со своим относительным приоритетом, а не только по
сравнению с какой-то фиксированной величиной как в случае с
``SCHED_RR``. Слово "должна" здесь употреблено не просто так, ибо
``vruntime`` по сути всего лишь моделирует идеальную ситуацию при
которой мы имеем процессор, не на что не "отвлекающийся" и работающий
с постоянной скоростью, что на практике далеко не так.

Для эффективного учёта значения ``vruntime`` каждой задачи CFS
использует структуру дерева (так называемое rbtree).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Автоматизация расстановки приоритетов
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
