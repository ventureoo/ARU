...
Группа пакетов ``base-devel`` **обязательна** к установке. Она
содержит ряд полезных инструментов без которых вы не сможете
установить множество пакетов из AUR, включая все те, о которых пойдет
речь далее в руководстве. И так как в начале руководства мы условились
пользоваться "дедовским" методом установки AUR пакетов, дополнительно
установим ``git`` для скачивания PKGBUILD. Далее в руководстве часто
будет требоваться редактировать те или иные конфигурационные файлы,
поэтому вам необходимо установить любой текстовый редактор на ваш вкус
и запускать его в терминале с правами root (то есть при помощи команды
...
Наиболее общей проблемой, которую можно выделить при выборе
используемого ПО, является дилемма консольного и графического
интерфейса. Консольный интерфейс для работы внутри терминала
является классическим для Unix-подобных систем, но при этом новые
пользователи Linux часто сторонятся и даже боятся работать с
командами, предпочитая использовать графический интерфейс, к которому
они привыкли из операционной системы Windows, и который является
основной парадигмой работы с ПК для большинства людей, т. е. "всё должно
решиться нажатием пары кнопок".

.. index:: wayland, x11, comparison
.. _wayland_vs_x11:
...
компонент, вы всегда можете найти любой нужный вам пакет через команду
``pacman -Ss``, которая в качестве аргументов принимает ключевые слова
для поиска. Если вы не смогли найти нужную вам программу в официальных
репозиториях, вы всегда можете найти всё что душе угодно в AUR (по
адресу https://aur.archlinux.org/packages/).
