...
.. warning:: Для систем с зашифрованным корневым разделом к
   ...
   Обратите внимание, что при этом вы должны указать в параметрах ядра
   ваш зашифрованный LUKS раздел, то есть ``rd.luks.name=UUID=root`` и
   `root=/dev/mapper/root` в конфигурации вашего загрузчика, а также
   желательно прописать в качестве значения параметру ``rootfstype``
   название вашей файловой системы, например ``rootfstype=ext4``.
