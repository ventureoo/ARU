...
.. warning:: Данный раздел предназначен для более-менее опытных
   пользователей Linux, тем не менее в нем автор немного упростил
   ...
   кадры, понятия устройства MMU и его таблицы, а также небольшое
   смешение понятий "процесс" и "программа", что, конечно, не совсем
   верно, так как "процесс != программа", но это необходимо для
   простоты понимания.
...
Фууух, что же, надеюсь вы не устали от всего этого скучного бреда выше
...
- Файловая "подложка" или *файловые страницы* - это страницы в которых
  ...
  понятие страничного кэша (page cache) [#]_. Если некоторая программа
  ...
  страниц выделяется по требованию, поэтому если программа открыла
  файл, но ничего из него не читает, то никакой реальной памяти для
  таких страниц выделено не будет. Собственно, то, что вы видите в
  графе "Кэш" в любой программе аналоге системного монитора в Linux -
  и есть страничный кэш. Обратите внимание, что исполняемые файлы
  (программы) тоже загружаются в память как файловые страницы.
...
найдет эту страницу, считает её из подкачки и обратно загрузит в
...
программе, то ядру придется их расжать перед использованием. Конечно,
...
.. info:: Некоторые пользователи задаются вопросом: В чем разница
   ...
   следующий цикл жизни: RAM -> Zswap -> Подкачка. Если программа
...
.. info:: Важно отметить, что для использования ZRAM вам вовсе не
...
чтение" (*readahead*), когда при обращении программы к вытесненной
...
фрагмент данных, которыми оперирует программа, поэтому с большой долей
вероятности обратившись к одной 4 Кб странице, программа сделает ещё
два и более запросов к тем страницам, которые тоже могли быть
вытеснены в подкачку и быть записанными в него после той, которую
программа запрашивает в данный момент, и чтобы их потом тоже не искать
и не читать ядро делает это сразу вместе с той вытесненной страницей,
которую запросила программа сейчас, так скажем, двух зайцев одним
выстрелом.
...
это будут страницы той же программы, которая запросила текущую
страницу, но на деле это может вообще не так. Ядро записывает страницы
из памяти в подкачку в том порядке, в котором они были вытеснены, и
они вообще не обязательно могут относится к одному и тому же процессу,
а даже если к одному, то могут быть совсем не теми, которые процесс
запросит в будущем. Короче говоря, с упреждающим чтением мы играем в
своего рода рулетку, повезет или нет. Но в подавляющем большинстве
случаев ядро просто вернет в память обратно ещё 8 страниц (согласно
значению по умолчанию), которые могут никогда не пригодиться в
будущем, а если они не пригодятся, то их придется опять вытеснять в
подкачку.
