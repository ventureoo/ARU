...
.. warning:: При сборки некоторых пакетов ``lto`` может вызвать
   регрессии, как например при сборке драйверов Mesa. Поэтому автор
   рекомендует его использовать выборочно.
