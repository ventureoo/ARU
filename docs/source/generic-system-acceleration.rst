...
.. attention:: Может понадобиться параметр PKGDEST для указания директории для собранного пакета. Вероятно без его задания нужно нужна будет пересборка (требует проверки).
