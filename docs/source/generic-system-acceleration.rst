...
каждой службы, но есть некоторые службы, такие как например
``systemd-networkd-wait-online`` или ``NetworkManager-wait-online``
все же лучше выключить, а также пользовательские службы рабочих
окружений KDE Plasma, GNOME и Cinnamon (если вы их не используете, то
можете просто пропустить разделы связанные с ними).
...
~2-4 секунды. Очевидно, это совсем не то, что нам хотелось бы видеть от
...
Для ``systemd-networkd`` существует аналогичная служба, и она
занимается тем же самым, поэтому её тоже рекомендуется отключить::
...
.. index:: services, gnome, gsd
.. _disabling_gsd_services:

----------------------------------------
Отключение пользовательских служб GNOME
----------------------------------------
...
Отключение службы уведомления о начале печати. Если нет принтера или
вам просто не нужны эти постоянные уведомления - отключаем::

  systemctl --user mask org.gnome.SettingsDaemon.PrintNotifications.service

Отключение службы управления цветовыми профилями GNOME. Отключив её не
будет работать тёплый режим экрана (Системный аналог Redshift)::
...
рекомендуется отключать для ноутбуков с активным использованием
Wi-Fi::
...
Отключение службы интеграции GNOME с карт-ридером. Если у вас
...
Будучи форком GNOME 3, Cinnamon также имеет собственный аналог GSD
служб, который называется службы CSD (Cinnamon Settings Daemon).
Принципиальных различий от GSD служб у них по сути нет, просто другое
название и немного измененный состав. Однако отличается способ,
которым можно их отключить, так как теперь нам нужно отключать их при
помощи переопределения файлов автозапуска::

  cd ~/.config/autostart
  cp -v /etc/xdg/autostart/cinnamon-settings-daemon-*.desktop .

``cinnamon-settings-daemon-wacom`` - служба интеграции Cinnamon с
графическим планшетом Wacom. Если у вас такого нет - смело
отключайте::
...
Отключение службы автоматической блокировки экрана::

  echo "Hidden=true" >> cinnamon-settings-daemon-screensaver-proxy.desktop

``cinnamon-settings-daemon-smartcard`` - служба интеграции Cinnamon с
карт-ридером. Если у вас карт-ридера нет, то смело отключайте::

  echo "Hidden=true" >> cinnamon-settings-daemon-smartcard.desktop

``cinnamon-settings-daemon-housekeeping`` - служба автоматического
оповещения вас о недостаточном количестве свободного места на диске.
Если вы делаете это самостоятельно при помощи специальных средств, как
например Baobab, то можно отключить данную службу::

  echo "Hidden=true" >> cinnamon-settings-daemon-housekeeping.desktop

``cinnamon-settings-daemon-orientation`` - служба настройки ориентации
дисплея. Если у вас нет сенсорного экрана или поддержки переворота
дисплея - отключайте::

  echo "Hidden=true" >> cinnamon-settings-daemon-orientation.desktop

``cinnamon-settings-daemon-power`` - служба настройки энергосбережения
Cinnamon. Смело отключаем, если у вас не ноутбук::

  echo "Hidden=true" >> cinnamon-settings-daemon-power.desktop

``cinnamon-settings-daemon-clipboard`` - службу интеграции работы
буфера обмена c Cinnamon. Если вы никогда не пользовались виджетом
истории буфера обмена в трее, то можете отключить::

  echo "Hidden=true" >> cinnamon-settings-daemon-clipboard.desktop

Если после отключения какой-либо из вышеперечисленных служб что-то
пошло не так, или просто какую-либо из них понадобилось снова
включить, то используйте следующую команду предварительно подставив
имя нужной службы::

  rm -rf ~/.config/autostart/cinnamon-settings-daemon-СЛУЖБА.desktop

Это вернет нужную службу в строй после перезагрузки оболочки.
