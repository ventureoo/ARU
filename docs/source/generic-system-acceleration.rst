...
==============================
Ускорение сжатия образов ядра
==============================

Начиная с 31 версии mkinitcpio использует алгоритм сжатия Zstd по умолчанию.
Zstd также поддерживает многопоточность, что может значительно ускорить
сжатие образов ядра.

Чтобы его задействовать отредактируем конфигурацию mkinitcpio::

  sudo nano /etc/mkinitcpio.conf

  # Найдите строку ниже и впишите в неё данное значение
  COMPRESSION_OPTIONS=(-T$(nproc))
