...
компановки в исполняемый файл или библиотеку. Для ускорения процесса
...
- Создать отдельную директорию *tmpfs* заданного размера: 
  ...
  - Необходимо в ``/etc/fstab`` добавить директорию для монтирования
    *tmpfs*, где указать место директории к которой подключить *tmpfs*
    и максимальный объём до которого может расширяться *tmpfs* (учтите
    что *tmpfs* использует ОЗУ, поэтому внимательно подходите к
    вопросу выделяемого объема, несмотря на то, что изначально *tmpfs*
    ничего не занимает), например::
...
.. attention:: На системах с небольшим количеством ОЗУ (например 4 ГБ)
   *tmpfs* может негативно сказаться на сборке тяжёлых пакетов, что
   может привести к недостатку ОЗУ для сборки.

.. attention:: Может понадобиться параметр PKGDEST для указания
   директории собранного пакета. Вероятно без его задания нужна
   будет пересборка (требует проверки).
...
Будет отображена информация почти о всех файоовых системах. В списке
только нужно будет найти необходимую строчку, либо добавить grep к
команде, например::
