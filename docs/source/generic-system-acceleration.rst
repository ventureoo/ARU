...
будет отображена информация почти о всех файоовых системах. В списке только нужно будет найти необходимую строчку либо добавить grep к команде, например::

  df -h | grep /var/tmp/makepkg 
