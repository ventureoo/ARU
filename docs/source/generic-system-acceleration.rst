...
но LLVM имеет чуть большее примущество с точки зрения производительности при меньших потерях в качестве конечного кода.
Поэтому, в целом, применение компиляторов LLVM для сборки различных пакетов при задании флага -O3
...
И отредактиурем значения флагов компилятора, CFLAGS и CXXFLAGS соответственно, на данные::

  CFLAGS="-fdiagnostics-color=always -pipe -O3 -march=native -fstack-protector-strong"
  CXXFLAGS="-fdiagnostics-color=always -pipe -O3 -march=native -fstack-protector-strong"
...
.. attention:: Далеко не все пакеты так уж гладко собираются через Clang вместе с применением максимального уровня оптимизации (-O3),
   в частности не пытайтесь собирать им Wine/DXVK, т.к. это официально не поддерживается и с 98% вероятностью приведет к ошибке сборки.
   Поэтому, в случае неудачи, вы всегда можете либо выбрать оптимальный уровень оптимизаций (флаг -O2), либо использовать компиляторы GCC,
   которые у вас заданы в настройках makepkg.conf по умолчанию, т.е. просто уберите опцию :bash:`--config /etc/makepkg-clang.conf` из команды
   :bash:`makepkg`.
...
который идёт напрямую из ядра и является самым быстром,
