...
Если проблема осталась, то можете попробовать снизить задержки, 
так как при маленьких нагрузках и повышенных изменениях ``quantum`` 
задержки повышаются, а из-за этого могут появиться хрипы
...
на изменения буффера (например Telegram), по этому вы можете попробовать
изменить ``min-quantum`` или вообще не прописывать его(по умолчанию 32),
а изменить только ``quantum`` и ``max-quantum``. Разное реагирование
приложений может происходить из-за того, что приложения могут блокировать
``quantum`` до определённых значений.
