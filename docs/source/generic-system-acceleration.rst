...
- Создать отдельную директорию *tmpfs* заданного размера: 
  ...
  - Необходимо в ``/etc/fstab`` добавить директорию для монтирования *tmpfs*, где указать место директории к которой подключить *tmpfs* и максимальный объём до которого может расширяться *tmpfs*, например::

     tmpfs   /var/tmp/makepkg         tmpfs   rw,nodev,nosuid,size=16G          0  0

  - Далее как и в предыдущем указать ``BUILDDIR`` в ``/etc/makepkg.conf``, но уже с путем к директории указанной в *fstab*::
   
     BUILDDIR=/var/tmp/makepkg makepkg
