...
.. attention:: На системах с небольшим количеством ОЗУ (например 4 ГБ) *tmpfs* может негативно сказаться на сборке тяжёлых пакетов, что может привести к недостатку ОЗУ для сборки. Может понадобиться параметр PKGDEST для указания директории для собранного пакета.
