...
.. warning:: Этот параметр допустепен только X11 сессии Plasma. Для
...
.. index:: lowlatency, compositor, muffin, x11-unrediction
.. _lowlatency-muffin:
...
Muffin. В нем по сути нет настроек, и его нельзя заменить на другой
композитор как мы это делали с Xfwm. Вся настройка Muffin сводиться к
одному параметру: *"Отключение композитора для полноэкранных окон"*.
